...
struct TrieNode{
    ...
    TrieNode<T> *tryStep( T c) {
        if (!chd.count(c)) return nullptr;
        return chd[c];
    }

    void insert(const vector<T> &vT, int pos) {
        if (pos == vT.size()) ed = true;
        else {
...
template<typename T>
struct Trie{
    TrieNode<T> *root, *curr; 
    Trie() root = new TrieNode<T>();
    void insert(const vector<T> &vT) root->insert(vT, 0);
    bool find(const vector<T> &vT) return root->find(vT, 0);
};
